if CLIENT then
         ...
         killicon.AddFont("hegrenade_thrown", "CSKillIcons", "h", Color(255, 80, 0, 255))
end
...
function SWEP:DrawWeaponSelection(x, y, wide, tall)
         draw.SimpleText("H", "CSSelectIcons", x + wide / 2, y + tall / 4, Color(255, 255, 0, 255), TEXT_ALIGN_CENTER)
end
...
function SWEP:Think()
         if self.Throw == 1 and self.ThrowTimer <= CurTime() and !self.Owner:KeyDown(IN_ATTACK) then
                  if SERVER then
                           local entity = ents.Create("hegrenade_thrown")
